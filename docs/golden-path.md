...
|---|---|---|---|
| 1. Load | Verify the full commit URL, then read only the allowlisted repository instructions and acceptance state. | Paste the XHS prompt. | Report the resolved 40-character commit and currently allowed modes. |
| 2. Install | Open the official ego(lite) quick-start page if the runtime is unavailable. Do not source a binary elsewhere. If the current Codex task retains an old/unavailable skill after installation, ask for one new task with the same XHS prompt. | Install the macOS app, finish GUI onboarding, optionally import Chrome data, quit any old duplicate, launch the official `/Applications` copy, and reply that onboarding is complete. | The running app and newly loaded skill match one complete runtime tuple; a minimal `ego-browser` call succeeds. |
...
| 5. Discover | Search only approved sites and recruitment types; normalize, deduplicate, and retain original links and timestamps. | Confirm cities, role, cohort, hard constraints, sites, and search budget. | Reviewable job table with hard-gate failures and uncertainty labels. |
| 6. Draft | Score selected jobs from evidence and create truthful, job-specific material drafts with a diff. | Select exactly one job and approve the material hash. | `批准草稿：<jobId>/<materialHash>` is recorded; no external disclosure has occurred. |
| 7. Fill | If the acceptance report enables real Fill, snapshot the exact employer, URL, fields, answers, files, hashes, and privacy effects. Fill only after exact authorization and stop before final submit. | Review the disclosure snapshot, reply `授权填写：<jobId>/<bindingHash>`, and personally handle sensitive/legal fields. | Filled values and attachments are read back; final submission has not occurred. |
| 8. Submit | If the acceptance report enables real Submit, revalidate the page, persist `submit_prepared`, request the exact second confirmation, persist `submit_started`, click once, and verify the result. | Reply `确认提交：<jobId>/<attemptId>/<bindingHash>` only after final review. | Local ledger records `submitted`, `failed`, `skipped`, or non-retryable `unknown`. |
...
Browser work is enabled only when the installed ego(lite) app version and the loaded `ego-browser` skill version/date/SKILL.md SHA256 exactly match one tuple in `config/skills.lock.yaml`. A successful smoke test alone is not an identity match.

The allowlist may contain multiple independently reviewed tuples, so a supported update does not require rewriting the workflow. An unknown, partially observed, or mismatched version fails closed for browser work; offline parsing/drafting may continue only when the Codex host still has an already trusted document capability. Review a new release, append its tuple, rerun acceptance, and publish a new pinned repository commit before enabling it.
